...
| EX-2 | Finalize Key Vault runtime wiring (`key_vault_uri`, secret-name mapping, access checklist) for Bronze notebooks | DemoOps / QA Engineer | Complete | All prerequisites verified: KV URI set, workspace identity active (a2e1a377-67a2-46e9-a0e4-20c69ea65bc4), RBAC granted, connection configured. Access checklist documented at `docs/keyvault-access-checklist.md`. All 8 Bronze notebooks updated to use live API mode with resolve_secret pattern. ✅ 2026-06-12 |
...
| EX-5 | Create Azure resource group and Key Vault, grant Fabric workspace identity secret-reader access, and store required API secrets with config handoff | Azure Infra Lead | Complete | Created `rg-transit-demo-kv` and `kvtransitdemo-f70cfb6a`, loaded MBTA/WMATA/Ticketmaster secrets, workspace identity enabled and active, RBAC assignment complete. All handoff items in `parameters.dev.yml`. ✅ 2026-06-12 |
