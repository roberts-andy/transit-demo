...
# - **No business logic**: Timestamp UTC conversion, `predicted_delay_seconds` derivation, attribute unwrapping, and conformed joins all belong in Silver — not here.
...
# 
# > **Note on ROUTE_FILTER**: `/predictions` is a high-cardinality endpoint. The MBTA API requires a `filter[route]` or `filter[stop]` parameter — calling without a filter returns HTTP 400 or an empty result set. Always provide `ROUTE_FILTER` in live mode.
...
SOURCE_SYSTEM   = "mbta"
BRONZE_TABLE    = "bronze_mbta_predictions_raw"
API_MODE        = "live"  # "live" | "cached"
FIXTURE_PATH    = "../../fixtures/bronze/mbta_predictions_sample.json"
# NOTE: /predictions is a high-cardinality endpoint and REQUIRES a route filter.
# The MBTA API returns HTTP 400 or empty results without filter[route] or filter[stop].
ROUTE_FILTER    = "Red"  # e.g. "Red", "Orange", "Blue", "Green-B", "1"
...
if API_MODE == "cached":
    ...
    # filter[route] is required — /predictions returns HTTP 400 without a filter
    url = f"{API_BASE_URL}{SOURCE_ENDPOINT}?filter[route]={ROUTE_FILTER}"
...
print(f"Extracted {len(records)} records from {SOURCE_SYSTEM}{SOURCE_ENDPOINT} (route filter: {ROUTE_FILTER})")
