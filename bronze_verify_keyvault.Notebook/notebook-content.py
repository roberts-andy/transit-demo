...
# Using Key Vault URI directly (not connection name)
KEY_VAULT_URI = "https://kvtransitdemo-f70cfb6a.vault.azure.net"  # NO trailing slash
...
print(f"Using Key Vault URI: {KEY_VAULT_URI}")
print()
...
try:
    ...
    print(f"MBTA: ****{mbta_key[-4:] if len(mbta_key) >= 4 else '****'}")
except Exception as e:
    results["mbta_secret"] = f"❌ FAIL"
    errors.append(f"MBTA: {str(e)}")
    print(f"MBTA failed: {str(e)[:150]}")

# Resolve WMATA secret
try:
    wmata_key = mssparkutils.credentials.getSecret(KEY_VAULT_URI, "wmata-api-key")
    results["wmata_secret"] = "✅ PASS"
    print(f"WMATA: ****{wmata_key[-4:] if len(wmata_key) >= 4 else '****'}")
except Exception as e:
    results["wmata_secret"] = f"❌ FAIL"
    errors.append(f"WMATA: {str(e)}")
    print(f"WMATA failed: {str(e)[:150]}")
